...
if (isset($_POST['generate_report'])) {
    ...
    // Save report generation as a notification
    $notification_query = $conn->prepare("INSERT INTO `notifications` (`seller_id`, `message`, `is_read`, `created_at`) VALUES (?, ?, 0, NOW())");
    $notification_message = "Generated sales report from $start_date to $end_date with total sales Rs. $total_sales.";
    $notification_query->execute([$seller_id, $notification_message]);
}
?>
...
<section class="report">
    ...
    <?php if (isset($report_query) && $report_query->rowCount() > 0) { ?>
        <div class="report-results">
            <h2>Sales Report from <?= htmlspecialchars($start_date); ?> to <?= htmlspecialchars($end_date); ?></h2>
            <p>Total Sales: Rs. <?= htmlspecialchars($total_sales); ?>/-</p>
            <table>
                <thead>
                    <tr>
                        <th>Order ID</th>
